---
source_file: "vendor/claude-mem/src/services/integrations/CodexCliInstaller.ts"
type: "code"
community: "Community None"
degree: 4
location: "L123"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_None
---

# installCodexCli()

> [!info] Properties
> **File Type**: code
> **Community**: [[_COMMUNITY_Community None|Community None]]
> **Degree**: 4

## Architecture Graph
```mermaid
graph LR
    center["installCodexCli[]"]:::centerNode
    center -->|"contains"| N_vendor_claude_mem_src_services_integrations_codexcliinstaller_ts["CodexCliInstaller.ts"]
    center -->|"calls"| N_integrations_codexcliinstaller_loadexistingtranscriptwatchconfig["loadExistingTranscriptWatchConfig[]"]
    center -->|"calls"| N_integrations_codexcliinstaller_mergecodexwatchconfig["mergeCodexWatchConfig[]"]
    center -->|"calls"| N_integrations_codexcliinstaller_writeconfigandshowcodexinstructions["writeConfigAndShowCodexInstructions[]"]
    classDef centerNode fill:#4E79A7,stroke:#fff,stroke-width:2px,color:#fff
```

## Outbound Connections
- [[CodexCliInstaller.ts]] - `contains` [EXTRACTED]
- [[loadExistingTranscriptWatchConfig()]] - `calls` [EXTRACTED]
- [[mergeCodexWatchConfig()]] - `calls` [EXTRACTED]
- [[writeConfigAndShowCodexInstructions()]] - `calls` [EXTRACTED]

## Inbound Dependencies
> [!abstract]- Files that link here
```dataview
LIST
FROM [[installCodexCli()]]
```

#graphify/code #graphify/EXTRACTED #community/Community_None
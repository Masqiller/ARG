---
source_file: "vendor/claude-mem/src/shared/find-claude-executable.ts"
type: "code"
community: "Community None"
degree: 8
location: "L72"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_None
---

# findClaudeExecutable()

> [!info] Properties
> **File Type**: code
> **Community**: [[_COMMUNITY_Community None|Community None]]
> **Degree**: 8

## Architecture Graph
```mermaid
graph LR
    center["findClaudeExecutable[]"]:::centerNode
    center -->|"contains"| N_vendor_claude_mem_src_shared_find_claude_executable_ts["find-claude-executable.ts"]
    center -->|"calls"| N_shared_find_claude_executable_lookslikedesktopapppath["looksLikeDesktopAppPath[]"]
    center -->|"calls"| N_shared_find_claude_executable_verifyclaudeversion["verifyClaudeVersion[]"]
    center -->|"imports"| N_vendor_claude_mem_src_services_worker_claudeprovider_ts["ClaudeProvider.ts"]
    center -->|"imports"| N_vendor_claude_mem_src_services_worker_knowledge_knowledgeagent_ts["KnowledgeAgent.ts"]
    center -->|"calls"| N_worker_claudeprovider_claudeprovider_startsession[".startSession[]"]
    center -->|"calls"| N_knowledge_knowledgeagent_knowledgeagent_prime[".prime[]"]
    center -->|"calls"| N_knowledge_knowledgeagent_knowledgeagent_executequery[".executeQuery[]"]
    classDef centerNode fill:#4E79A7,stroke:#fff,stroke-width:2px,color:#fff
```

## Outbound Connections
- [[.executeQuery()]] - `calls` [INFERRED]
- [[.prime()]] - `calls` [INFERRED]
- [[.startSession()]] - `calls` [INFERRED]
- [[ClaudeProvider.ts]] - `imports` [EXTRACTED]
- [[KnowledgeAgent.ts]] - `imports` [EXTRACTED]
- [[find-claude-executable.ts]] - `contains` [EXTRACTED]
- [[looksLikeDesktopAppPath()]] - `calls` [EXTRACTED]
- [[verifyClaudeVersion()]] - `calls` [EXTRACTED]

## Inbound Dependencies
> [!abstract]- Files that link here
```dataview
LIST
FROM [[findClaudeExecutable()]]
```

#graphify/code #graphify/EXTRACTED #community/Community_None
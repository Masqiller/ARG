...
```mermaid
graph LR
    center["applyClaudeCodePathSetupIfNeeded[]"]:::centerNode
    center -->|"contains"| N_vendor_claude_mem_src_npx_cli_commands_install_ts["install.ts"]
    center -->|"calls"| N_commands_install_detectshellconfigfile["detectShellConfigFile[]"]
    classDef centerNode fill:#4E79A7,stroke:#fff,stroke-width:2px,color:#fff
```

## Outbound Connections
- [[detectShellConfigFile()]] - `calls` [EXTRACTED]
...
```dataview
LIST
FROM [[applyClaudeCodePathSetupIfNeeded()]]
```

...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        if (name === "read_god_nodes") {
            ...
            if (!fs.existsSync(reportPath)) return ok("GRAPH_REPORT.md not found. Run graphify first.");
        ...
        if (name === "get_node_neighbors") {
            ...
            if (!fs.existsSync(graphPath)) return ok("graph.json not found.");
            
            const graph = JSON.parse(fs.readFileSync(graphPath, "utf-8"));
            const links = graph.links || [];
            const neighbors = links.filter((l: any) => l.source === nodeId || l.target === nodeId);
        ...
        if (name === "get_graph_stats") {
            ...
            const stats = {
                ...
                topNodes: nodes.slice(0, 10).map((n: any) => n.id || n.name)
            };
            return ok(JSON.stringify(stats, null, 2));
        }
        
        if (name === "read_wiki_article") {
            const article = args.article_name as string;
            const wikiPath = path.join(graphifyOut, "wiki", article.endsWith('.md') ? article : `${article}.md`);
            if (!fs.existsSync(wikiPath)) return ok(`Wiki article ${article} not found.`);
        ...
        if (name === "recommend_skills") {
            const recommended = recommendSkills(graphifyOut);
            return ok(formatScoredSkills(recommended));
        }
        
        if (name === "recommend_skills_for_task") {
            const task = args.task as string;
            const recommended = recommendSkillsForTask(graphifyOut, task);
            return ok(formatScoredSkills(recommended));
        }
        ...
        if (name === "read_skill") {
            ...
            if (!skillPath) return ok(`Skill '${skillName}' not found in the library.`);
        ...
        if (name === "generate_agent_briefing") {
            const task = args.task as string;
            const maxSkills = (args.max_skills as number) || 3;
        ...
        if (name === "decompose_for_parallel") {
            ...
            if (fs.existsSync(graphPath)) {
                ...
                communities = Array.from(communityMap.entries()).map(([key, files]) =>
                    `Community ${key}: ${files.slice(0, 5).join(', ')}${files.length > 5 ? ` (+${files.length - 5} more)` : ''}`
                );
            }
        ...
        if (name === "arg_memory_retrieve") {
            ...
            if (!argBrain) {
                argBrain = new ARGBrain(graphifyOut);
            }
            ...
            if (result.relevantMemories && result.relevantMemories.length > 0) {
                ...
                result.relevantMemories.forEach((mem: any) => {
            ...
            if (result.relevantNodes && result.relevantNodes.length > 0) {
                ...
                result.relevantNodes.forEach((node: any) => {
...
function formatScoredSkills(skills: ScoredSkill[]): string {
    if (skills.length === 0) return "No skills recommended. Run graphify first to generate structural data.";
